...
function BasicPanel({
  ...
  function handleContinue() {
    onContinue(
      categories.map((name) => ({ categoryName: name, parentName: null })),
    );
  }
  ...
  return (
    <div>
      <p className="text-sm text-gray-500 mb-4">
        Review the categories we found in your uploaded transactions and confirm
        they look right.
      </p>
...
function AdvancedPanel({
  ...
  // Group locked CSV categories by their parent from the existing taxonomy.
  // The parent may not itself appear in the CSV.
  const lockedChildToParent = new Map<string, string>();
  for (const existingCat of existing) {
    for (const child of existingCat.children ?? []) {
      if (categories.includes(child.name)) {
        lockedChildToParent.set(child.name, existingCat.name);
      }
    }
  }
  const lockedParentGroups = new Map<string, string[]>();
  for (const [child, parent] of lockedChildToParent) {
    if (!lockedParentGroups.has(parent)) lockedParentGroups.set(parent, []);
    lockedParentGroups.get(parent)!.push(child);
  }
  // Locked top-level categories that appear in the CSV also render as group
  // containers so unlocked items can be dropped into them.
  for (const existingCat of existing) {
    if (
      categories.includes(existingCat.name) &&
      !lockedParentGroups.has(existingCat.name)
    ) {
      lockedParentGroups.set(existingCat.name, []);
    }
  }
  const inLockedGroup = new Set(lockedChildToParent.keys());
  // Locked top-level parents live in the groups section, not the pool.
  const lockedTopLevelParents = new Set(
    existing.filter((c) => categories.includes(c.name)).map((c) => c.name),
  );
  ...
  // Categories sitting in the unassigned pool (not a child, not a promoted parent, not explicitly
  // promoted, not in a locked parent group, and not a locked top-level parent itself)
  const pool = categories.filter(
    (c) =>
      !(c in assignments) &&
      !promotedParents.includes(c) &&
      !userParents.includes(c) &&
      !inLockedGroup.has(c) &&
      !lockedTopLevelParents.has(c),
  );

  const allLocked =
    categories.length > 0 && categories.every((c) => lockedNames.has(c));
  ...
  function handleDropOnPoolItem(target: string) {
    if (!dragging || dragging === target || allParents.includes(dragging))
      return;
    setAssignments((prev) => {
      const next = { ...prev };
  ...
  return (
    <div>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="flex flex-col gap-1.5">
            ...
            <p className="text-xs text-gray-600">
              <span className="font-semibold">Drag and drop</span> an ungrouped
              item onto a group card to add it as a sub-category.
            </p>
          </div>
          <div className="flex flex-col gap-1.5">
            {/* Folder-plus icon */}
            ...
            <p className="text-xs text-gray-600">
              <span className="font-semibold">Parent Categories</span> can be
              created manually or by dragging an item into the New Group area.
            </p>
          </div>
          <div className="flex flex-col gap-1.5">
            {/* Lock icon */}
            ...
            <p className="text-xs text-gray-600">
              <span className="font-semibold">Locked groups</span> are pre-set
              and can&apos;t be changed, but you can add sub-categories to
              locked parents.
            </p>
          </div>
        </div>
      ...
      <div className="flex items-center gap-3 mb-2">
        ...
        {allLocked && (
          <p className="text-xs text-warning">
            Cannot be modified, pre-existing.
          </p>
        )}
      </div>
      <div
      ...
      {lockedParentGroups.size === 0 && allLocked ? (
        <p className="text-sm text-warning italic">Cannot be modified.</p>
      ) : (
        <div className="grid grid-cols-2 gap-4">
          {/* Locked parent groups derived from the existing taxonomy */}
          {[...lockedParentGroups.entries()].map(([parent, lockedChildren]) => {
            const userChildren = getChildren(parent);
            const totalCount = lockedChildren.length + userChildren.length;
            return (
              <div
                key={`__locked_parent__${parent}`}
                onDragOver={(e) => {
                  e.preventDefault();
                  setDragOver(parent);
                }}
            ...
            allParents.map((parent, index) => {
              ...
              return (
                ...
                >
                  {/* Card header */}
                  <div
                    className={`${headerColor} rounded-t-lg px-4 py-2.5 flex items-center justify-between`}
                  >
          ...
          {/* New group card — hidden when all CSV categories are pre-existing */}
          {!allLocked && (
            <div
              onDragOver={(e) => {
                e.preventDefault();
                setDragOver("__create__");
              }}
              onDragLeave={() => setDragOver(null)}
              onDrop={(e) => {
                e.preventDefault();
              ...
              onClick={() => {
                if (!addingGroup) setAddingGroup(true);
              }}
